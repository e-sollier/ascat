...
ascat.plotRawData = function(ASCATobj, img.dir=".", img.prefix="") {
  ...
  for (i in 1:dim(ASCATobj$Tumor_LogR)[2]) {
    png(filename = file.path(img.dir, paste(img.prefix, ASCATobj$samples[i],".tumour.png",sep="")), width = 32000, height = 16000, res = 3200)
  ...
  if(!is.null(ASCATobj$Germline_LogR)) {
    ...
    for (i in 1:dim(ASCATobj$Germline_LogR)[2]) {
      png(filename = file.path(img.dir, paste(img.prefix, ASCATobj$samples[i],".germline.png",sep="")), width = 32000, height = 16000, res = 3200)
      par(mar = c(0.5,5,5,0.5), mfrow = c(2,1), cex = 0.4, cex.main=3, cex.axis = 2, pch = ifelse(dim(ASCATobj$Tumor_LogR)[1]>100000,".",20))
...
ascat.plotSegmentedData = function(ASCATobj, img.dir=".", img.prefix="") {
  for (arraynr in 1:dim(ASCATobj$Tumor_LogR)[2]) {
    ...
    png(filename = file.path(img.dir, paste(img.prefix, ASCATobj$samples[arraynr],".ASPCF.png",sep="")), width = 32000, height = 16000, res = 3200)
  ...
  if(nropt>0) {
    ...
    if (is.na(nonroundedprofilepng)) {
      ...
      if(pdfPlot){
        ...
        png(filename = nonroundedprofilepng, width = 32000, height = (y_limit*800), res = 3200)
      }
    }
    ...
    if (is.na(copynumberprofilespng)) {
      ...
      if(pdfPlot){
        ...
        png(filename = copynumberprofilespng, width = 32000, height = (y_limit*1600), res = 3200)
      }
    }
    #plot ascat profile
...
ascat.predictGermlineGenotypes = function(ASCATobj, platform = "AffySNP6", img.dir=".", img.prefix="") {
  ...
  for (i in 1:dim(ASCATobj$Tumor_LogR)[2]) {
    ...
    png(filename = file.path(img.dir,paste(img.prefix, "tumorSep",colnames(ASCATobj$Tumor_LogR)[i],".png",sep="")), width = 32000, height = 8000, res = 3200)
