...
ascat.plotRawData = function(ASCATobj, img.dir=".", img.prefix="") {
  ...
  for (i in 1:dim(ASCATobj$Tumor_LogR)[2]) {
    png(filename = file.path(img.dir, paste(img.prefix, ASCATobj$samples[i],".tumour.png",sep="")), width = 64000, height = 32000, res = 6400)
  ...
  if(!is.null(ASCATobj$Germline_LogR)) {
    ...
    for (i in 1:dim(ASCATobj$Germline_LogR)[2]) {
      png(filename = file.path(img.dir, paste(img.prefix, ASCATobj$samples[i],".germline.png",sep="")), width = 64000, height = 32000, res = 6400)
      par(mar = c(0.5,5,5,0.5), mfrow = c(2,1), cex = 0.4, cex.main=3, cex.axis = 2, pch = ifelse(dim(ASCATobj$Tumor_LogR)[1]>100000,".",20))
...
ascat.plotSegmentedData = function(ASCATobj, img.dir=".", img.prefix="") {
  for (arraynr in 1:dim(ASCATobj$Tumor_LogR)[2]) {
    ...
    png(filename = file.path(img.dir, paste(img.prefix, ASCATobj$samples[arraynr],".ASPCF.png",sep="")), width = 64000, height = 32000, res = 6400)
  ...
  if(nropt>0) {
    ...
    if (is.na(nonroundedprofilepng)) {
      ...
      if(pdfPlot){
        ...
        png(filename = nonroundedprofilepng, width = 64000, height = (y_limit*1600), res = 6400)
      }
    }
    ...
    if (is.na(copynumberprofilespng)) {
      ...
      if(pdfPlot){
        ...
        png(filename = copynumberprofilespng, width = 64000, height = (y_limit*3200), res = 6400)
      }
    }
    #plot ascat profile
...
ascat.predictGermlineGenotypes = function(ASCATobj, platform = "AffySNP6", img.dir=".", img.prefix="") {
  ...
  for (i in 1:dim(ASCATobj$Tumor_LogR)[2]) {
    ...
    png(filename = file.path(img.dir,paste(img.prefix, "tumorSep",colnames(ASCATobj$Tumor_LogR)[i],".png",sep="")), width = 64000, height = 16000, res = 6400)
...
ascat.predictGermlineGenotypes_NGS = function(ASCATobj, maxHomozygous=0.025,proportionHetero=0.15,proportionHomo=0.80,proportionOpen=0.05,segmentLength=100, img.dir=".", img.prefix="") {
  ...
  Hetero = 0
    ...
    png(filename = file.path(img.dir,paste(img.prefix, "tumorSep",colnames(ASCATobj$Tumor_LogR)[i],".png",sep="")), width = 16000, height = 4000, res = 1600)
    title = paste(paste(colnames(ASCATobj$Tumor_BAF)[i], Hetero), Homo)
    ascat.plotGenotypes(ASCATobj,title,Tumor_BAF_noNA, Hom, ch_noNA)
    dev.off()
